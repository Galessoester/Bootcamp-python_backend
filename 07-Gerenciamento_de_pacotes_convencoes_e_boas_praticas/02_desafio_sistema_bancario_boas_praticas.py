...
def log_transacao(func):
    def envelope(*args, **kwargs):
        ...
        with open(ROOT_PATH / "log.txt", "a", newline="", encoding="utf-8") as log:
            log.write(
                f"[{data_hora}] Função '{func.__name__}' executada com argumentos {args} e {kwargs}. Retornou {funcao}\n"
            )
...
def atualizar_arquivo_contas(contas):
    with open(ROOT_PATH / "contas.csv", "a", newline="", encoding="utf-8") as arquivo:
        ...
        if isinstance(contas, list):
            for conta in contas:
                writer.writerow(
                    [conta.agencia, conta.numero, conta.cliente.cpf, conta.saldo]
                )
        else:
            writer.writerow(
                [contas.agencia, contas.numero, contas.cliente.cpf, contas.saldo]
            )
